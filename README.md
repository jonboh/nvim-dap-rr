An extension for [nvim-dap](https://github.com/mfussenegger/nvim-dap) to generate ready to go rr configurations.
...
To configure the debugger configuration you can use `get_config(debuggerOpts)`:
```lua
table.insert(dap.configurations.cpp, rr_dap.get_config(
    {
        miDebuggerPath = "<path>",
...
```
The entries that you specify will overwrite the defaults:
```lua
...
```
**Warning:**
The plugin relays in the name of the configuration to setup the mappings of its stepping functions.
...
If you have an unrelated dap config with the same name as a rr config this will make sessions
